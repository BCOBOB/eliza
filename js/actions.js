...
function getUserInput(){
	...
	if(inputFromUser == null || inputFromUser != null && inputFromUser.length == 0){
		...
		sendFrasierNewMessage(inputFromUser);
	}
}
/*
...
function displayChat(){
	...
	if(chatHistory.length == 0){
		...
		for(var i = 0;i < chatHistory.length;i++){
			...
			if(currentMessage.isFrasier){
				html += getFrasierMessageHTML(currentMessage.content);
			}else{
...
/*
 * HTML for Frasier's message
 */
function getFrasierMessageHTML(message){
	return '<div class="chat-message-outter-wrapper"><img src="./assets/imgs/guy.png" class="chat-message-icon"><div class="chat-message-wrapper eliza-message"><p class="chat-message-text text-left">' + message + '</p><p class="chat-user-text">Frasier</p></div></div>';
}
/*
