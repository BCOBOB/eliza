 ...
 * Frasier. This decides the initial message.
 *
...
function startFrasierChat(){
	...
	chatHistory.push({ isFrasier : true, content : initialMessage });
	displayChat();
...
/*
 * SendFrasierNewMessage is a function used for a one input, and one response
 * output. The idea is; here is where it gets the output from analyze but
 * this function manipulates the UI.
 */
function sendFrasierNewMessage(newMessage){
	
	//Add to UI
	chatHistory.push({ isFrasier : false, content : newMessage });
	displayChat();
	clearSendTextbox();
	...
	//Shows the reply from Frasier. To make it seem more real, takes a
	//random short time to response. Reloads the chat.
	setTimeout(function(){
		usedResponses.push(response);
		chatHistory.push({ isFrasier : true, content : response });
		displayChat();
	}, determineResponseTime());

}
/*
 * Analyze is Frasier's main function get a response based on
